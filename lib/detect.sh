...
# Select REQUESTED as the project workdir after resolving symlinks. The stored
# path is physical and must remain inside the physical Git root. Outputs are
# globals because command substitution would hide them in a subshell.
_kyzn_select_project_workdir() {
    local root="$1"
    local requested="$2"
    ...
    if [[ "$requested" == /* || "$requested" == ".." || "$requested" == ../* || \
        ...
        KYZN_PROJECT_WORKDIR_ERROR="project.root '$requested' must be a relative path without '..' components"
        return 1
    fi

    root_real=$(cd "$root" 2>/dev/null && pwd -P) || {
        KYZN_PROJECT_WORKDIR_ERROR="repository root is unavailable: $root"
        return 1
    }
    candidate_real=$(cd "$root/$requested" 2>/dev/null && pwd -P) || {
        KYZN_PROJECT_WORKDIR_ERROR="project.root '$requested' does not name an existing directory"
        return 1
    }
    ...
    case "$candidate_real" in
        "$root_real")
            ...
            ;;
        "$root_real"/*)
            KYZN_PROJECT_SUBDIR="${candidate_real#"$root_real"/}"
            ;;
        *)
            KYZN_PROJECT_WORKDIR_ERROR="project.root '$requested' resolves outside the repository"
            return 1
            ;;
    esac

    KYZN_PROJECT_WORKDIR="$candidate_real"
...
detect_project_type() {
    ...
    if [[ -n "$configured_root" ]]; then
        if _kyzn_select_project_workdir "$root" "$configured_root"; then
    ...
    [[ "$explicit_root_valid" == true ]] && detect_root="$KYZN_PROJECT_WORKDIR"

    _kyzn_detect_manifests_at "$detect_root"

    # Nothing at the root, and no explicit project.root pinning it there
    # — check one level down, generalizing the old Cargo-workspace-only glob
    # to every language. Invalid explicit configuration never reaches this
    # fallback: silently selecting another directory would violate the user's
    # explicit choice. Ambiguous auto-detection stays generic.
    if [[ "$explicit_root_valid" == false && -z "$KYZN_PROJECT_TYPE" ]]; then
        ...
        if (( ${#candidates[@]} == 1 )); then
            if _kyzn_select_project_workdir "$root" "${candidates[0]}"; then
                _kyzn_detect_manifests_at "$KYZN_PROJECT_WORKDIR"
            else
                log_error "$KYZN_PROJECT_WORKDIR_ERROR"
            fi
    ...
    if [[ -z "$KYZN_PROJECT_TYPE" ]]; then
        KYZN_PROJECT_TYPE="generic"
        KYZN_PROJECT_TYPES+=("generic")
        if [[ "$explicit_root_valid" == false ]]; then
            KYZN_PROJECT_SUBDIR=""
            KYZN_PROJECT_WORKDIR=""
        fi
    fi
