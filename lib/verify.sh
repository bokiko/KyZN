...
# ---------------------------------------------------------------------------
# Failures are deliberately NOT classified here.
#
# Invariant: authorization depends only on the final verify_build exit code.
# Classifying failure OUTPUT must never authorize success.
#
# Rationale and the toolchain cases behind it: test 81, docs/how-it-works.md.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Capture failing test names — DIAGNOSTICS AND PROMPTS ONLY.
#
# No authorization decision reads this. It is captured once at baseline so the
# run can show the operator which tests were already red, and so the fix prompt
# can tell Claude which failures it did not cause. Best-effort by construction:
# an incomplete or empty list degrades the message, never the verdict.
#
