...
# ---------------------------------------------------------------------------
test_provider_resolve_claude() {
    log_header "69. resolve_provider: claude resolves when CLI exists"

    source "$KYZN_ROOT/lib/provider.sh"

    # claude is installed on this machine
    local result
    result=$(resolve_provider "claude" 2>/dev/null)
    assert_eq "resolve_provider claude" "claude" "$result"
}

test_provider_resolve_codex_missing() {
    log_header "70. resolve_provider: codex fails when CLI missing"

    source "$KYZN_ROOT/lib/provider.sh"

    if command -v codex &>/dev/null; then
        skip "codex is installed — cannot test missing path"
        return
    fi

    local rc=0
    resolve_provider "codex" &>/dev/null || rc=$?
    assert_eq "resolve_provider codex (missing)" "1" "$rc"
}

test_provider_resolve_auto() {
    log_header "71. resolve_provider: auto resolves deterministically"

    source "$KYZN_ROOT/lib/provider.sh"

    # On this machine, claude exists so auto should resolve to claude
    ...
    local result
    result=$(resolve_provider "auto" 2>/dev/null) || true
    # Auto should pick claude first (deterministic order)
    assert_eq "resolve_provider auto" "claude" "$result"
}

test_provider_resolve_invalid() {
    log_header "72. resolve_provider: invalid provider fails"

    source "$KYZN_ROOT/lib/provider.sh"

    local rc=0
    resolve_provider "gpt4all" &>/dev/null || rc=$?
    assert_eq "resolve_provider invalid" "1" "$rc"
}

test_provider_model_mapping() {
    log_header "73. resolve_provider_model: maps KyZN hints to provider models"
    ...
    # Codex maps to OpenAI models
    assert_eq "codex sonnet→o4-mini" "o4-mini" "$(resolve_provider_model codex sonnet)"
    assert_eq "codex opus→o3" "o3" "$(resolve_provider_model codex opus)"
}

test_provider_validate_output_findings() {
    log_header "74. validate_output: findings_json contract enforcement"

    source "$KYZN_ROOT/lib/provider.sh"

    # Valid JSON array should pass
    local rc=0
    validate_output "findings_json" '[{"id":"SEC-001"}]' 2>/dev/null || rc=$?
    assert_eq "valid findings array" "0" "$rc"

    # Invalid (object, not array) should fail
    rc=0
    validate_output "findings_json" '{"id":"SEC-001"}' 2>/dev/null || rc=$?
    assert_eq "invalid findings object" "1" "$rc"

    # Empty array should pass
    rc=0
    validate_output "findings_json" '[]' 2>/dev/null || rc=$?
    assert_eq "empty findings array" "0" "$rc"

    # Not JSON at all should fail
    rc=0
    validate_output "findings_json" 'hello world' 2>/dev/null || rc=$?
    assert_eq "non-json findings" "1" "$rc"
}

test_provider_validate_output_empty() {
    log_header "75. validate_output: empty output fails closed"

    source "$KYZN_ROOT/lib/provider.sh"

    local rc=0
    validate_output "findings_json" "" 2>/dev/null || rc=$?
    assert_eq "empty output fails" "1" "$rc"

    rc=0
    validate_output "improve_json" "" 2>/dev/null || rc=$?
    assert_eq "empty improve output fails" "1" "$rc"
}

test_provider_config_precedence() {
    log_header "76. provider config: CLI flag overrides config"

    create_sandbox "generic"

    source "$KYZN_ROOT/lib/core.sh"

    # Write config with codex
    ensure_kyzn_dirs
...
YAML
    ...
    local from_config
    from_config=$(config_get '.preferences.provider' 'claude')
    assert_eq "config reads codex" "codex" "$from_config"

    # Simulate CLI override
    local provider_from_cli="claude"
    local requested="${provider_from_cli:-$(config_get '.preferences.provider' 'claude')}"
    assert_eq "CLI overrides config" "claude" "$requested"

    # Default when no CLI and no config
    provider_from_cli=""
    cat > "$KYZN_CONFIG" <<'YAML'
project:
  type: generic
preferences:
  mode: deep
YAML
    requested="${provider_from_cli:-$(config_get '.preferences.provider' 'claude')}"
    assert_eq "default is claude" "claude" "$requested"

    cleanup_sandbox
}

test_provider_credential_paths_blocked() {
    log_header "77. security: Codex/OpenAI credential paths in KYZN_SETTINGS_JSON"

    source "$KYZN_ROOT/lib/core.sh"

    # Verify Codex/OpenAI paths are blocked
    assert_contains "codex creds blocked" "$KYZN_SETTINGS_JSON" ".codex/"
    assert_contains "openai config blocked" "$KYZN_SETTINGS_JSON" "openai/"
}
...
main() {
    ...
    # Provider tests
    echo ""
    echo -e "${BOLD}${CYAN}━━━ Provider adapter tests ━━━${RESET}"
    test_provider_resolve_claude
    test_provider_resolve_codex_missing
    test_provider_resolve_auto
    test_provider_resolve_invalid
